...
summary: Changing the admin password, two-factor sign-in, break-glass codes, and the factory reset. Under Appliance and Data, then Access and Security.
...
- Type the 6-digit code the app shows, and press **Verify and Enable**. Until you do, nothing changes.
...
- If you lose the authenticator and the secret, the way back is to delete data/local-config.json on the server (see Signing in).

## Break-glass codes

A break-glass code belongs to one owner. It starts with **bg-** and works in place of the admin password, recorded against that owner's name. Settings cannot make one yet: the break-glass card under Access and Security describes a plan and its button does nothing. Codes are only issued through the server's API, to owners.
...
**Wipe and Reset Node** needs the admin password. It empties data/local-config.json: the admin password, two-factor sign-in, the community's name and contact details, the gateway settings, the money thresholds and the backup settings. It does **not** delete members, posts, deals or beans, and it keeps the community's own key. After a restart the server takes ADMIN_PASSWORD from .env again, or makes up a new one and prints it in the log.
