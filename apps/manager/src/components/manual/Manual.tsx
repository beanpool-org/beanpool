...
export function ManualPanel({ slug, onNavigate, onClose, pages }: { slug: string | null; onNavigate: (slug: string | null) => void; onClose: () => void; pages?: readonly string[] }) {
    ...
    const shown = useCallback((p: OperatorGuidePage | null): p is OperatorGuidePage => p !== null && (!pages || pages.includes(p.slug)), [pages]);
    const page = slug && shown(manualPage(slug)) ? manualPage(slug) : null;
    const results = useMemo(
        () => (query.trim() ? searchGuide(OPERATOR_MANUAL, query).filter(r => shown(r.page as OperatorGuidePage)) : []),
        [query, shown],
    );
