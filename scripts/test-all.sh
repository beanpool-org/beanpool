...
run_federation_suites() {
  bash -c '
    ...
    FAILED=""
    for t in test-schema-upgrade test-recovery-shares test-sso test-keeper-deposit test-commons-conservation test-demurrage-window test-crowdfund-delete-refund test-admin-password-query test-cors-policy test-gateway-config test-csrf-protection test-totp-admin-2fa test-moderation-admin test-ledger-audit-startup test-mirror-sync-audit-log test-federation-bridge test-connector-credit-cap test-connector-public-url test-federation-link test-listing-reach test-listing-pull test-settlement-state test-settlement-exchange test-settlement-orchestration test-federation-purchase-route test-federation-commission test-federation-settlement test-apple-probe; do
      echo "━━━ $t ━━━"
      TMP_DIR=$(mktemp -d)
