...
function getStmtActiveMemberCount() {
    if (!_stmtActiveMemberCount) {
        _stmtActiveMemberCount = db.prepare(
            "SELECT COUNT(*) as c FROM posts WHERE author_pubkey != ? AND active = 1 AND status = 'active' AND type NOT IN ('poll', 'event') AND (audience_scope IS NULL OR audience_scope = 'public')"
        );
    }
...
function getStmtActiveCountAll() {
    if (!_stmtActiveCountAll) {
        _stmtActiveCountAll = db.prepare(
            "SELECT COUNT(*) as c FROM posts WHERE active = 1 AND status = 'active' AND type NOT IN ('poll', 'event') AND (audience_scope IS NULL OR audience_scope = 'public')"
        );
    }
