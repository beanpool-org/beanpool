 ...
 * top of `GET /api/marketplace/posts`, `GET /api/community/members`, and `GET /api/activity/feed`
 * to answer a conditional request with a 304 *before* touching SQLite, so every write path that
 * changes one of those responses has to be able to bump them — including low-level engine modules
 * like `engine/members.ts` or `db/activity-feed-db.ts`, which cannot import `state-engine.ts`
 * without a cycle.
 *
 ...
 * adding a write path to `posts`, `members`, or `activity`, bump it here or route it through `broadcast()`.
 */

let postsVersion = Date.now();
let membersVersion = Date.now();
let activityVersion = Date.now();
...
export function bumpActivityVersion(): number {
    return ++activityVersion;
}
