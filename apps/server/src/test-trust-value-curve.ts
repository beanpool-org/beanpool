 ...
 *   8. Vouch gate: only an appointed voucher's vouch (or a grant) hands out the -20 floor — trading alone does not.
 *
...
function main() {
    ...
    assert(floorOf('buyer3') === -21, `once vouched → buyer floor −21 (20 voucher + 1 earned unlocked), got ${floorOf('buyer3')}`);
    assert(floorOf('seller3') === -21, `the seller ALSO earned it → floor −21 once vouched, got ${floorOf('seller3')}`);
    ...
    assert(floorOf('farmer') === -35, `40 × 1-bean completed trades, vouched → floor −35 (20 voucher + 15 earned, not −2000), got ${floorOf('farmer')}`);
    ...
    assert(floorOf('trader') === -568, `2000 across 5 sellers, vouched → floor −568 (20 voucher + 548 earned), got ${floorOf('trader')}`);
    ...
    assert(floorOf('whale') === -980, `10k with one partner capped at 5k, vouched → floor −980 (20 voucher + 960 earned), got ${floorOf('whale')}`);
    ...
    assert(floorOf('vouchedFresh') === -20, `vouched, no trades → floor −20 (welcome voucher only), got ${floorOf('vouchedFresh')}`);
    ...
    assert(floorOf('didTrade') === -980, `vouched + 5000 trade → trading floor −980, got ${floorOf('didTrade')}`);
