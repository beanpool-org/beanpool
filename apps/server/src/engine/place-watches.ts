 ...
 *   watch one of them reaches hears once for the run: one push and one live announcement to their own sockets,
 *   naming the nearest. A community is new once in the life of the database, so nobody hears about it twice; a
 *   watch set after a community was first seen never hears about it (it was already there to find).
...
interface WatchRecord { id: string; pubkey: string; lat: number; lng: number; radius_km: number; created_at: string }
...
export function setPlaceWatch(pubkey: string, point: { lat: number; lng: number }, radiusKm: number): { watch: PlaceWatch; created: boolean } {
    ...
    return db.transaction(() => {
        ...
        const row: WatchRecord = { id: crypto.randomUUID(), pubkey, lat, lng, radius_km: radiusKm, created_at: new Date().toISOString() };
...
/** A re-key: the member's watches move to their new key (a cell both keys watch is kept once). */
...
function named(name: string | null): string {
    return name ? `"${name}"` : 'A new community';
}

export function communityNearBody(name: string | null, km: number): string {
    return `${named(name)} is now in the BeanPool directory, about ${km} km from a place you're watching.`;
}

export function communitiesNearBody(count: number, nearestName: string | null, km: number): string {
    return `${count} communities are now in the BeanPool directory near a place you're watching. The nearest, ${named(nearestName)}, is about ${km} km away.`;
}

/**
 * Tells each member whose watch one of `added` reaches, once: one push (marketplace category, so their Marketplace
 * notification setting applies) and one `system_announcement` to their own sockets. Only watches set before `now`
 * count (all of them, in a run: the communities are new at `now`). Returns how many members were told.
 */
export function notifyPlaceWatchers(cb: PlaceWatchNoticeCallbacks, added: readonly DirectoryRow[], now: string): number {
    ...
    for (const [pubkey, found] of byMember) {
        ...
        const body = near.length === 1 ? communityNearBody(near[0].c.name, km) : communitiesNearBody(near.length, near[0].c.name, km);
